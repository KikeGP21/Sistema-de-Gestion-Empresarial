...
def lectorPDF():
    ...
    extraccion_datos = open('Datos cliente.txt', 'r', encoding="utf-8")
...
def mariaDB():
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO cliente (RAZON_SOCIAL,NIF,CODIGO_DE_INSCRIPCION,DOMICILIO,POBLACION,MUNICIPIO,PROVINCIA,PAIS,CP,NOMBRE,APELLIDO,SEXO,DNI,CALIDAD,CORREO,TELEFONO) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    connection.commit()
    print('\n' + '\033[92m' + "Los datos del cliente se han insertado en la bbdd satisfactoriamente." + '\033[0m')
    connection.close()
